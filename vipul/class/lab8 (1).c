...
struct point2d fp(struct point2d **par,struct point2d a)
{
  if(par[a.x][a.y].x==a.x&&par[a.x][a.y].y==a.y)
  {
    return a;
  }
  else
...
void uni(struct point2d **mst,struct point2d **par,struct point2d a,struct point2d b,int *cnt)
{
  ...
  if(pa.x!=pb.x||pa.y!=pb.y)
  {
    ...
    mst[*cnt][1].y=b.y;
    (*cnt)++;
    par[pa.x][pa.y].x=pb.x;
    par[pa.x][pa.y].y=pb.y;
  }
}
void euclideanMST(struct point2d **mst,struct point2d *points,int n)
{
  ...
  for(i=0;i<n;i++)
  {
    mxx=(points[i].x>mxx)?points[i].x:mxx;
    mxy=(points[i].y>mxy)?points[i].y:mxy;
    //printf("%d %d %d %d %f\n",arr[i].a.x,arr[i].a.y,arr[i].b.x,arr[i].b.y,arr[i].dis);
  }
  struct point2d **par;
  par=(struct point2d **)malloc((mxx+1)*sizeof(struct point2d *));
  for(i=0;i<=mxx;i++)
  {
     par[i]=(struct point2d *)malloc((mxy+1)*sizeof(struct point2d));
  }
  for(i=0;i<n;i++)
  {
    par[points[i].x][points[i].y].x=points[i].x;
    par[points[i].x][points[i].y].y=points[i].y;
  }
  int cnt=0;
  for(i=0;i<num;i++)
  {
  	printf("%d %d %d %d %f\n",arr[i].a.x,arr[i].a.y,arr[i].b.x,arr[i].b.y,arr[i].dis);
    uni(mst,par,arr[i].a,arr[i].b,&cnt);
  }
  printf("MST\n");
  for(i=0;i<n-1;i++)
  {
